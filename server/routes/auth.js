...
function isProfileComplete(user) {
  return user.name && user.email && user.studentId && user.phone && user.department && user.address;
}

// Google OAuth Strategy
passport.use(new GoogleStrategy({
  clientID: process.env.GOOGLE_CLIENT_ID, // Loaded from environment variable
  clientSecret: process.env.GOOGLE_CLIENT_SECRET, // Loaded from environment variable
  ...
  try {
    ...
    if (!user) {
      user = new User({
        ...
        profileComplete: false // will be updated below
      });
      user.profileComplete = isProfileComplete(user);
      await user.save();
      // Send welcome email
      try { await sendWelcomeEmailNodemailer(user.email, user.name); } catch (e) { console.error('Welcome email error:', e); }
    } else {
...
// Add endpoint to check profile completeness and list missing fields
router.put('/complete-profile', protect, async (req, res) => {
  try {
    ...
    if (missingFields.length > 0) {
      return res.json({ success: false, missingFields });
    }
    return res.json({ success: true, message: 'Profile is complete' });
  } catch (err) {
