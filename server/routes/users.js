...
router.get('/marketplace-stats', protect, async (req, res) => {
  try {
    ...
    // Total sales (products sold by user, only completed)
    const totalSales = await Transaction.countDocuments({ seller: userId, status: 'completed' });
    // Total purchases (products bought by user, only completed)
    const totalPurchases = await Transaction.countDocuments({ buyer: userId, status: 'completed' });
    // Seller rating (average of all ratings for this seller)
    const user = await User.findById(userId);
    const sellerRating = user?.sellerRating || 0;
    // Total transactions (as buyer or seller, only completed)
