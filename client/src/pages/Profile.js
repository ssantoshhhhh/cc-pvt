...
const Profile = () => {
  ...
  const [profileComplete, setProfileComplete] = useState(user?.profileComplete);
  const [missingData, setMissingData] = useState({});
  const [missingFields, setMissingFields] = useState([]);

  useEffect(() => {
    setProfileComplete(user?.profileComplete);
    const requiredFields = ['studentId', 'phone', 'department', 'address'];
    const missing = {};
    requiredFields.forEach(field => {
      if (!user?.[field]) missing[field] = '';
    });
    setMissingData(missing);

    // Fetch missing fields from backend
    const fetchMissingFields = async () => {
      try {
        const res = await axios.put('/api/auth/complete-profile', {}, { withCredentials: true });
        if (res.data && res.data.missingFields && res.data.missingFields.length > 0) {
          toast.error(
            `Please update your profile. Missing fields: ${res.data.missingFields.map(f => f.charAt(0).toUpperCase() + f.slice(1)).join(', ')}`
          );
        }
      } catch (err) {
  ...
  const handleCompleteProfile = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.put('/api/auth/complete-profile', missingData, { withCredentials: true });
      if (res.data.success) {
  ...
  const handleMissingChange = (e) => {
    setMissingData({ ...missingData, [e.target.name]: e.target.value });
  };
  ...
  const googleProfilePicture = user?.googleProfilePicture || null;

  // Determine missing required fields
  const requiredFields = ['phone', 'address'];
  const missingFieldsList = requiredFields.filter(field => !user?.[field]);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="flex items-center space-x-4">
              ...
              {missingFieldsList.length > 0 && (
                <div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-4" role="alert">
                  ...
                  <p>Please fill in the following required fields: {missingFieldsList.join(', ')}</p>
                </div>
              )}
              <div>
          ...
          <div className="p-6">
            ...
            {/* Removed the form for profile completion */}
